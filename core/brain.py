...
# ── Clientes LLM desde services/ ──────────────────────────────────────────────
try:
    from services.ollama_client import OllamaClient as _OllamaServiceClient
    from services.gemini_client import GeminiClient as _GeminiServiceClient
    from services.groq_client import GroqClient as _GroqServiceClient
    from services.openai_client import OpenAIClient as _OpenAIServiceClient
    from services.mistral_client import MistralClient as _MistralServiceClient
    _SERVICES_OK = True
except ImportError:
    _SERVICES_OK = False
...
class SmartRouter:
    ...
    Prioridad (Req 5.1):
      ...
      Mistral → respaldo cloud antes de caer a local
      Ollama  → fallback local si no hay internet o OFFLINE_MODE=True
    """
    ...
    def __init__(self):
        ...
        # Tarea 4.2 / 4.3: estado de carga lazy de Ollama
        self._ollama_loaded: bool = False
        self._last_online_state: Optional[bool] = None

    def analyze(self, query: str) -> RoutingDecision:
        # ── Tarea 4.4: Respetar OFFLINE_MODE ─────────────────────────────────
        try:
            from config.settings import OFFLINE_MODE, PREFER_CLOUD
        except Exception:
            OFFLINE_MODE = False
            PREFER_CLOUD = True
        ...
        if OFFLINE_MODE:
            ...
            return RoutingDecision("ollama", 1.0, "OFFLINE_MODE forzado")

        # ── Tarea 4.3: Detectar recuperación de internet → descargar Ollama ──
        current_online = self._connectivity.is_online()
        if (self._last_online_state is False and current_online is True
                and self._ollama_loaded):
            self._unload_ollama()
        self._last_online_state = current_online

        # ── Sin internet → carga lazy de Ollama (Tarea 4.2) ──────────────────
        ...
        if time.time() < self._gemini_blocked_until:
            # Si PREFER_CLOUD, intentar Groq/Mistral antes de Ollama
            if PREFER_CLOUD:
                return RoutingDecision("groq", 0.9, "gemini bloqueado → groq")
        ...
        # ── Tarea 4.4: Respetar MAX_RAM_MB ───────────────────────────────────
        try:
            from config.settings import MAX_RAM_MB
            import psutil, os as _os
            proc = psutil.Process(_os.getpid())
            ram_mb = proc.memory_info().rss / (1024 * 1024)
            if ram_mb > MAX_RAM_MB:
                print(f"[SmartRouter] RAM {ram_mb:.0f}MB > {MAX_RAM_MB}MB → priorizando cloud")
                # Forzar cloud para liberar presión de RAM
                return RoutingDecision("groq", 0.95,
                                       f"RAM alta ({ram_mb:.0f}MB) → groq")
        except Exception:
            pass

        # ── Orden de prioridad cloud: Gemini → Groq → Mistral → Ollama ───────
    ...
    def should_unload_ollama(self) -> bool:
        """
        Retorna True cuando Ollama debe descargarse de memoria (Tarea 4.3).
        Condición: antes estaba offline (Ollama cargado) y ahora está online,
        y PREFER_CLOUD es True.
        """
        try:
            from config.settings import PREFER_CLOUD
        except Exception:
            PREFER_CLOUD = True
        return (
            self._ollama_loaded
            and self._connectivity.is_online()
            and PREFER_CLOUD
        )

    def get_cloud_fallback_order(self) -> list:
        """
        Retorna el orden de prioridad de modelos cloud (Tarea 4.1).
        Gemini → Groq → Mistral
        """
        return ["gemini", "groq", "mistral"]

    def _unload_ollama(self) -> None:
        """Descarga el cliente Ollama de memoria cuando vuelve internet (Tarea 4.3)."""
        import gc
        self._ollama_loaded = False
        gc.collect()
        print("[SmartRouter] 🔄 Internet recuperado — Ollama descargado de memoria")
    ...
    def mark_gemini_failed(self) -> None:
        ...
        print("[SmartRouter] Gemini bloqueado 60s — usando Groq/Mistral")
...
class HybridIntelligenceCore:
    ...
    def __init__(self):
        ...
        # Tarea 4.2: Ollama se carga de forma lazy — solo cuando no hay internet
        # o cuando OFFLINE_MODE=True. No se instancia al inicio.
        self._ollama: Optional[OllamaEngine] = None
        ...
        print(f"[Brain]   Ollama  : lazy (se carga solo sin internet)")
    ...
    def _get_ollama(self) -> OllamaEngine:
        """
        Carga lazy del cliente Ollama (Tarea 4.2).
        Solo se instancia cuando se necesita (sin internet o OFFLINE_MODE).
        """
        if self._ollama is None:
            self._ollama = OllamaEngine()
            print("[Brain] 🦙 Ollama cargado en memoria (modo offline)")
        return self._ollama
    ...
    def _release_ollama(self) -> None:
        ...
        if self._ollama is not None:
            self._ollama = None
            gc.collect()
            print("[Brain] 🔄 Ollama descargado de memoria (internet recuperado)")
    ...
    def _ejecutar_tool_calls(self, content: str, user_input: str) -> str:
        ...
        for nombre, params_str in matches:
            try:
                ...
                try:
                    from core.security_manager import check_action
                    desc = params.get("descripcion", params.get("ruta", nombre))
                    if not check_action(nombre, str(desc)):
                        print(f"[Brain] 🔒 Acción '{nombre}' cancelada por SecurityManager")
                        tool_call_str = f"TOOL_CALL: {nombre}({params_str})"
                        resultado_final = resultado_final.replace(tool_call_str, "")
                        continue
                except Exception:
                    pass  # fail-silent: si security falla, continúa
    ...
    def _generate(self, decision: RoutingDecision,
        ...
        Prioridades (Tarea 4.1 — Gemini → Groq → Mistral → Ollama):
          gemini  → Groq → Mistral → Ollama
          groq    → Gemini → Mistral → Ollama
          openai  → Gemini → Groq → Mistral → Ollama
          ollama  → Ollama (sin internet / OFFLINE_MODE)
        ...
        # Tarea 4.3: Si SmartRouter detecta que volvió internet, liberar Ollama
        if self.router.should_unload_ollama():
            self._release_ollama()

        engine = decision.engine

        # Tarea 4.2: Carga lazy de Ollama — solo cuando se necesita
        if engine == "ollama":
            ollama_eng = self._get_ollama()
            if ollama_eng.is_available():
                try:
                    content = ollama_eng.generate(messages)
        ...
        # Cadenas de failover según motor principal (Tarea 4.1: orden Gemini→Groq→Mistral→Ollama)
        chains = {
            "gemini":  ["gemini", "groq", "mistral", "ollama"],
        ...
        # Tarea 4.4: Si PREFER_CLOUD=True, excluir Ollama de la cadena cuando hay internet
        try:
            from config.settings import PREFER_CLOUD
            if PREFER_CLOUD and self.router._connectivity.is_online():
                chain = [e for e in chain if e != "ollama"]
                if not chain:
                    chain = ["groq", "gemini", "mistral"]
        except Exception:
            pass
        ...
        engines_map = {
            ...
            "openai":  self._openai,
            # Ollama se resuelve lazy en el loop
        }
        ...
        for eng_name in chain:
            # Resolución lazy para Ollama
            if eng_name == "ollama":
                eng = self._get_ollama()
            else:
                eng = engines_map.get(eng_name)
                if eng is None:
                    continue
        ...
        # Último recurso: intentar Ollama si no estaba en la cadena
        ollama_eng = self._get_ollama()
        if not ollama_eng.is_available():
...
def get_security_manager():
    ...
    Wrapper de conveniencia para acceder al SecurityManager desde brain.py
    sin importar directamente core.security_manager en cada módulo.

    Returns:
        Instancia única de SecurityManager.
    """
    try:
        from core.security_manager import get_security_manager as _get_sm
        return _get_sm()
    except Exception:
        # fail-silent: si no se puede importar, retornar None
        return None
